...
template<typename T>
class List2 // класс двусвязный циклический список
{
private:
...
public:
	...
	T removeAT(const int index); // удалить указанный элемент 
...
T List2<T>::removeAT(const int index) // удалить указанный элемент 
{
...
T& List2<T>::sel_el(const int index) const 
{
	if (index > this->size - 1) // Если указанный индекс находится вне границ списка
	{
		std::cout << "Указанный индекс находится вне границ! Аварийное завершение операции" << std::endl;
		abort();
	}
